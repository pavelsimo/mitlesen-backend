"""Spanish sentence segmentation using spaCy."""
...
class SpanishSentenceSegmenter(BaseSegmenter):
    """Spanish sentence segmenter using spaCy for natural language processing."""

    def __init__(self):
        """Initialize the Spanish segmenter with spaCy model."""
        # ──────────────────────────────────────────────────────────────────────────────
        # INITIALISATION
        # ──────────────────────────────────────────────────────────────────────────────
        try:
            ...
            logger.warning("Spanish spaCy model not found. Downloading …")
    ...
    def split_long_sentence(self, sent: spacy.tokens.Span, max_len: int = 100) -> List[str]:
        ...
        Similar to German implementation but adapted for Spanish punctuation patterns.
        Handles Spanish-specific punctuation like ¿?, ¡!, and common Spanish 
        sentence structures.
        ...
        for tok in tokens:
            ...
            # Check for Spanish-friendly split points: commas, semicolons, colons
            if tok.strip() and tok.strip()[-1] in ",;:":
    ...
    def segment_text(self, text: str, max_len: int = 60) -> List[str]:
        ...
        Handles Spanish sentence boundaries including inverted question marks (¿?) 
    ...
    def _extract_text_from_words(self, words: List[Dict[str, Any]]) -> str:
        """Extract text using Spanish space-separated concatenation."""
        return " ".join(w["text"] for w in words)

    def _align_words_to_sentence(self, sentence: str, words: List[Dict[str, Any]], start_idx: int) -> Tuple[List[Dict[str, Any]], int]:
        """
        Align words to sentence using Spanish normalization-based matching.
        
        Uses normalize_spanish_text() for fuzzy matching to handle spacing, 
        punctuation, and Spanish accent differences.
        """
        target_norm = normalize_spanish_text(sentence)
        sentence_start_idx = None

        # Locate sentence start
        for i in range(start_idx, len(words)):
            if normalize_spanish_text(" ".join(w["text"] for w in words[i:])).startswith(target_norm):
        ...
        for j in range(sentence_start_idx, len(words)):
            word = words[j]["text"]
            running_norm = normalize_spanish_text((running_norm + " " + word).strip())
    ...
    def _get_processing_log_prefix(self) -> str:
        """Get Spanish-specific log prefix."""
        return "Processing (Spanish) "